...
// ==== CONFIG ====
const MENUS     = window.MENU_GROUPS; // จาก menu.js
...
function renderMenuList() {
  ...
  container.innerHTML = MENUS.map(group => {
...
function renderCart() {
  ...
  document.getElementById('cart').innerHTML = rows+`<div id="total">รวม ${total} ฿</div>` 
    || "<div style='color:#bbb'>ยังไม่มีสินค้าในตะกร้า</div>";
  document.getElementById('cart-count').textContent = count||'';
  document.getElementById('order-btn').disabled = count===0;
}
window.addToCart = (name,price)=>{ cart[name]=(cart[name]||{qty:0,price}).qty+=1; renderCart(); shakeCart(); }
window.setQty    = (name,v)=>{ cart[name].qty=Math.max(1,Math.min(99,Number(v))); renderCart(); }
window.removeFromCart = name=>{ if(--cart[name].qty<=0) delete cart[name]; renderCart(); }
...
window.scrollToCart = ()=>document.getElementById('cart').scrollIntoView({behavior:'smooth'});
...
async function fetchOrderHistory(){
  const table = getTableNumber();
  if(!table) return;
  const url = `${scriptURL}?table=${table}`;
  ...
  if(!Object.keys(summary).length){
    ...
    for(const [m,i] of Object.entries(summary)){
      html+=`<div>${m} <b>x${i.qty}</b> <span style="color:#2362aa;">${i.qty*i.price} ฿</span></div>`;
    }
    const total=Object.values(summary).reduce((s,i)=>s+i.qty*i.price,0);
    html+=`<div style="text-align:right;margin-top:5px;font-weight:bold;color:#1566a4;">รวม ${total} ฿</div>`;
  }
